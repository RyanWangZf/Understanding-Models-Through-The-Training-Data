...
from influence.inception_resnet_v2 import InceptionResNetV2
...
class BinaryInceptionResNetModel(GenericNeuralNet):
    ...
    def __init__(self, img_side, num_channels, weight_decay, **kwargs):
        ...
        super(BinaryInceptionResNetModel, self).__init__(**kwargs)
    ...
    def load_resnet_weights(self):
        ...
        weights_path = '/home/radaimi/Documents/influence-release-master/inception_resnet/inception_resnet_v2_weights_tf_dim_ordering_tf_kernels_notop.h5'
        self.inception_resnet_model.load_weights(weights_path)


    def inference(self, input):
        print(self.img_side)
        self.inception_resnet_model = InceptionResNetV2(include_top=False, weights='imagenet', input_tensor=tf.reshape(input, [-1, self.img_side, self.img_side, self.num_channels]))
        
        raw_inceptionresnet_features = self.inception_resnet_model.output

        pooled_inceptionresnet_features = AveragePooling2D((8, 8), strides=(8, 8), name='avg_pool')(raw_inceptionresnet_features)
        self.inception_features = Flatten(name='flatten')(pooled_inceptionresnet_features)
    ...
    def train_with_SGD(self, **kwargs):
        super(BinaryInceptionResNetModel, self).train(**kwargs)


    def minibatch_inception_resnet_features(self, feed_dict):
        ...
        for i in xrange(num_iter):
            ...
            ret[start:end, :] = self.sess.run(self.resnet_features, feed_dict=batch_feed_dict)            
    ...
    def train_with_LBFGS(self, feed_dict, save_checkpoints=True, verbose=True):
        ...
        X_train = self.minibatch_inception_resnet_features(feed_dict)
